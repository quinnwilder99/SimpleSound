 ...
 *
 * A Hilt-provided singleton so [com.simplesound.app.SimpleSoundApp] and
 * [com.simplesound.app.playback.PlaybackService] share the same instance instead
 * of each constructing their own (both still point at the same underlying
 * DataStore file either way, but sharing the instance means one settings change
 * is visible to both without waiting on a fresh collector).
 */
@Singleton
class SettingsStore
    @Inject
    constructor(
        ...
        private fun defaultTabs(): List<TabSetting> = Tab.Default.map { TabSetting(it, enabled = true) }
